...
training_df = training_df[,-c(1,3:Non_Probes)]
...
### Define Class Labels
class_labels <- as.factor(training_cor_df[,1])
...
rfe_output <- rfe(training_cor_df[,-1], class_labels, sizes=subsets, rfeControl=control)

#Save output files
save(rfe_output, rfe_output_prepro, file=paste(P1_output_dir,"Train_and_Test_data.Rdata",sep=""), compress = T)
...
plot(rfe_output_prepro, type = c("l"),col="red", main="Recursive Feature Elimination (Scaled / Centered)")
dev.off()
