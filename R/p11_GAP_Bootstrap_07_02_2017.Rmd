...
# 3 duplicate by 100
# 4 add column called ConfusionMatrix
# 5 100 vector, with "Phenotype" + True or False based on %
# 6 Rbind
...
PhenoMC_Data[28:35]<-apply(PhenoMC_Data[28:35], 2, round, digits = 2)
PhenoMC_Data[28:35]<-apply(PhenoMC_Data[28:35], 2, round, digits = 2)
PhenoMC_Data$Acc_2_Gx_Scz

# Calculate 
filter(PhenoMC_Data,Acc_1_Gx < 0.3375)
filter(PhenoMC_Data,Acc_1_Gx < 0.1)
names(metric_plot2)
color_metric="Percentage"
str(metric_plot2)




quantile(PhenoMC_Data$Acc_1_Gx)

filter(PhenoMC_Data,Acc_1_Gx > 0.98)
filter(PhenoMC_Data,Acc_2_Gx_Scz < 0.1)

##############
test<-filter(PhenoMC_Data,Phenotype == "FEP")
test
...
lVars <- c("Gender","Age", "Ethnicity","BMI","Tobacco")
...
table_dem_fun(PhenoMC_Data,table_name,stratVar,lVars,cVars)
table_name <-"Table_2_Clinicalinformation.csv"
table_dem_fun(PhenoMC_Data,table_name,stratVar,lVars2,cVars2)


table(filter(test2,rowIndex == 1)$Predict)
