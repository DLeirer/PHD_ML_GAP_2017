...
#Model Matrix
design = model.matrix(~0+fa_Gender+fa_Ethnicity)  
...
Demographics<-cbind(GX_DF_adj[,c(4,6)],design[,1:5])
PRS_Demo<-cbind(GX_DF_adj[,c(4,6,24)],design[,1:5])
...
ModelNum=3
set.seed(seed)
glmnet_3_PRS <- train(Phenotype~., data=predictorlists[[ModelNum]], method="glmnet", metric=metric, preProc=c("center", "scale"), trControl=control) 
#Save Models
...
glmvarimp<-varImp(glmnet_4_Dem, scale = FALSE)
