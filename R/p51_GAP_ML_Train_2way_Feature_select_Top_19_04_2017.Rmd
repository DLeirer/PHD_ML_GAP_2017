...
num<-Gx_DF_2_cor[,4]
var(num, y = NULL, na.rm = FALSE, use = "everything")
var(Gx_DF_2_cor[1:10,2:4], y = NULL, na.rm = FALSE, use = "everything")
...
modelID<-c("lda","glm","gbm","glmnet","nb","knn","svmRadial","rf","C5.0","elm","pcaNNet")
x=1
List_of_models_tune<-list()
List_of_models_tune[[modelID[x]]]<-"NA: Probable error"
List_of_models_tune[[modelID[x]]]<- train(Phenotype~., data=input_data, method=modelID[x], metric=metric, preProc=c("center", "scale"),tuneLength = 10, trControl=control)
print(List_of_models_tune[[modelID[x]]])
x=x+1
List_of_models_tune[[modelID[x]]]<-"NA: Probable error"
List_of_models_tune[[modelID[x]]]<- train(Phenotype~., data=input_data, method=modelID[x], metric=metric, preProc=c("center", "scale"),tuneLength = 10, trControl=control)
print(List_of_models_tune[[modelID[x]]])
...
for(i in 1:length(modelID)){
  tryCatch({
    ...
    set.seed(seed)
    List_of_models2[[modelID[i]]] <- train(Phenotype~., data=List_of_DFs[[RNum]], method=modelID[i], metric=metric, preProc=c("center", "scale"), trControl=control)
