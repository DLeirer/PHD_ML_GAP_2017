...
PRS_full_impute<-cbind(GX_DF_adj[,c("Phenotype","Ethnicity","ICD_DSM","PRS_0.1_adj","sampleID")],imputedesign[,2:6])
...
#Get NAs
PRS_NAs<-PRS_full_impute[is.na(PRS_full_impute$PRS_0.1_adj),]
...
GX_DF_adj$PRS_0.1<-resultspred$PRS_0.1_adj
...
ggplot(data = resultspred, 
       aes(x = ICD_DSM, y=PRS_0.1_adj  , colour = Imputed)) +
  geom_jitter() +
  geom_boxplot()+
...
#Define all 8 models with predictors 
N_index_probe<-37
Gx <-GX_DF_adj[,-c(1:3,5:N_index_probe)]
...
Gx_PRS <- cbind(PRS,GX_DF_adj[,-c(1:N_index_probe)])
Gx_Demo <- cbind(Demographics,GX_DF_adj[,-c(1:N_index_probe)])
Gx_Demo_PRS <- cbind(PRS_Demo,GX_DF_adj[,-c(1:N_index_probe)])
...
GX_data<-GX_DF_adj[1:N_index_probe]
...
#Define col indexes for percentage correct classification in glmnet
names(PhenoMC_Data)
col_index_percentages<-c(38:45)

######################Density plots
#######All models
metric_plot2<-melt(PhenoMC_Data, id=c(names(PhenoMC_Data[,-col_index_percentages])))
#Change colnames
colnames(metric_plot2)<-c(names(PhenoMC_Data[,-col_index_percentages]),"Model","Percentage")
#Control vs FEP
...
PhenoMC_Data$Acc_3_PRS
plot_fun_gx(PhenoMC_Data,"ICD_DSM","Acc_1_Gx","ICD_DSM","Ethnicity","Titleofplot")
plot_fun_gx(PhenoMC_Data,"ICD_DSM","Acc_3_PRS","ICD_DSM","Ethnicity","Titleofplot")
plot_fun_gx(PhenoMC_Data,"ICD_DSM","Acc_4_Demographics","ICD_DSM","Ethnicity","Titleofplot")
...
names(pansNAremovedNAPRS2)
cor(pansNAremovedNAPRS2$PanssScore,pansNAremovedNAPRS2[,c(18:37)])
