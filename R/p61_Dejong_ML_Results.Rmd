...
###### Predict GAP using category by class. 
Test_data_1 <- GX_DF_GAP
Dejong_results_ensemble2<-list() #empty list wrapper. 
confuse_ensemble2<-list() #empty list wrapper. 
for (i in 1:Number_of_Splits){
  #fold
  x=i
  #set top split ID
  ...
  confuse_ensemble2[[top_split]]<-confusionMatrix(Dejong_results_ensemble2[[top_split]], Test_data_1$Phenotype)
  print(confuse_ensemble2[[top_split]])
}

full_data$Split_2$
#Extract Metrics for table for all splits. 
confuse_ensemble2[[1]]$overall
...
full_data$Split_1$ensemble_test_performance


#predict(model_list[[1]],Test_data_1)



model_preds <- lapply(model_list, predict, newdata=Test_data_1, type="prob")
model_preds <- lapply(model_preds, function(x) x[,"Control"])
model_preds <- data.frame(model_preds)
model_preds3 <- model_preds
model_preds3$ensemble <- predict(gbm_ensemble, newdata=Test_data_1, type="prob")
caTools::colAUC(model_preds3, Test_data_1$Phenotype)

model_preds <- lapply(model_list, predict, newdata=Test_data_1)
model_preds <- data.frame(model_preds)
model_preds$ensemble <- predict(gbm_ensemble, newdata=Test_data_1)
model_preds$Phenotype<-GX_DF2$Phenotype
model_preds[1:10,]
model_preds3[1:10,]


model_preds3$Phenotype<-GX_DF2[,1]
model_preds3$ICD_DSM<-GX_DF2[,2]
model_preds3$ICD_DSM<-GX_DF2[,2]
model_preds3$ICD10_GAP<-GX_DF_adj$icd10
model_preds3$Ethnicity<-GX_DF_adj$Ethnicity


write.table(model_preds[1:10,],file = "Cat.csv")
write.table(round(model_preds3[1:10,],2),file = "percentage.csv")
model_preds[1:10,]
