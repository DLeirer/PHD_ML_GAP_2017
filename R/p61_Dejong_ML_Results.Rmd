...
#stats MAD https://eurekastatistics.com/using-the-median-absolute-deviation-to-find-outliers/
#clean data
MAD_input<-data.frame(MLresults_table)
MAD_input$Split<-rownames(MAD_input)
MAD_input<-melt(MAD_input, id=c("Split"))

#perform Mad calculation
MAD_input <- MAD_input[!is.na(MAD_input[,3]),]
MAD_input$Mad<-abs(MAD_input[,3] - median(MAD_input[,3])) / mad(MAD_input[,3])
MAD_input[MAD_input$Mad > 2,]
#group and average
MAD_input %>% group_by(Split) %>% summarise(average = mean(Mad))
MAD_input %>% group_by(variable) %>% summarise(average = mean(Mad))
histogram(MAD_input$Mad)
...
#stats MAD https://eurekastatistics.com/using-the-median-absolute-deviation-to-find-outliers/
#clean data
MAD_input<-data.frame(MLresults_table_GAP)
MAD_input$Split<-rownames(MAD_input)
MAD_input<-melt(MAD_input, id=c("Split"))
...
########## Added table just schizophrenia in both ICD10 and DSMIV
Split_1_Data_SCZ_both<-filter(Split_1_Data,(icd10 == "Schizophrenia" & dsmiv ==  "Schizophrenia") |  ICD_DSM == "Control")
GAP_SCZ_ICD_and_DSM<-f_meta_confuse(Split_1_Data_SCZ_both,"SCZ_Both")
write.csv(GAP_SCZ_ICD_and_DSM,paste(output_dir,project_id,project_name,Sub_prefix,"Split_1_Confusion_Matrix_SCZ_both_metrics.csv",sep=""))
25/30
...
Split_1_Data %>% group_by(Phenotype) %>% summarise(median(ensemble))
...
# Test Case - FEP
t.test(filter(Split_1_Data,ICD_DSM == "Control")$ensemble,filter(Split_1_Data,ICD_DSM != "Control")$ensemble)
t.test(filter(Split_1_Data,ICD_DSM == "Control")$ensemble,filter(Split_1_Data,ICD_DSM == "Schizophrenia")$ensemble)
t.test(filter(Split_1_Data,ICD_DSM == "Control")$ensemble,filter(Split_1_Data,ICD_DSM == "Other_Psychosis")$ensemble)
t.test(filter(Split_1_Data,ICD_DSM == "Schizophrenia")$ensemble,filter(Split_1_Data,ICD_DSM == "Other_Psychosis")$ensemble)
Other_var<-filter(Split_1_Data,ICD_DSM == "Other_Psychosis")$ensemble
SCZ_var<-filter(Split_1_Data,ICD_DSM == "Schizophrenia")$ensemble
Control_var<-filter(Split_1_Data,ICD_DSM == "Control")$ensemble

#Anova and Tukey test for diagnosis
fit <- aov(ensemble ~ ICD_DSM, data=Split_1_Data)
summary(fit)
Stat_results<-TukeyHSD(fit)
Stat_results

fit <- aov(ensemble ~ Gender + Ethnicity, data=Split_1_Data)
summary(fit)
Stat_results<-TukeyHSD(fit)
Stat_results

#Other Psych stats
Other_psych_stats<-filter(Split_1_Data,ICD_DSM != "Control")
Other_psych_stats<-filter(Split_1_Data,ICD_DSM != "Control")
#perform Mad calculation
#MAD_input <- Other_psych_stats[!is.na(MAD_input[,3]),]
Other_psych_stats$Mad<-abs(Other_psych_stats$ensemble - median(Other_psych_stats$ensemble)) / mad(Other_psych_stats$ensemble)
Other_psych_stats[Other_psych_stats$Mad > 1,]
Misclassed<-Other_psych_stats[Other_psych_stats$ensemble < 0.5,]

table(Other_psych_stats$dsmiv)
table(Misclassed$dsmiv)
table(Misclassed$icd10)
table(Misclassed$dsmiv)
table(Misclassed[,c("icd10","dsmiv")])
Otherpsych_ensemble<-filter(Other_psych_stats[,c("icd10","dsmiv","ICD_DSM","ensemble","ensemble_Categorical")],dsmiv == "Schizophreniform Disorder")
Otherpsych_ensemble<-filter(Other_psych_stats[,c("icd10","dsmiv","ICD_DSM","ensemble","ensemble_Categorical")],dsmiv == "Schizophrenia")
table(filter(Otherpsych_ensemble,ICD_DSM == "Schizophrenia")$ensemble_Categorical)
16/(9+16)
26/(26+6)
median(filter(Otherpsych_ensemble,ICD_DSM == "Schizophrenia")$ensemble)

filter(Split_1_Data,ICD_DSM != "Control")
dftest<-Split_1_Data %>% group_by(dsmiv) %>% filter(n()>15) #
table(dftest$dsmiv)
fit <- aov(ensemble ~ dsmiv, data=dftest)
summary(fit)
Stat_results<-TukeyHSD(fit)$dsmiv
colnames(Stat_results)= c("diff","lwr", "upr" ,"p_adj")
Stat_res_dsmiv<-data.frame(Stat_results)
Stat_res_dsmiv$compare= rownames(Stat_results)

filter(data.frame(Stat_res_dsmiv),p_adj <= 0.6)



#group and average
Other_psych_stats %>% group_by(dsmiv) %>% summarise(average = mean(Mad))
Other_psych_stats %>% group_by(dsmiv) %>% summarise(average = median(ensemble))
MAD_input %>% group_by(variable) %>% summarise(average = mean(ensemble))
Other_psych_stats
...
f_plots_posthoc(Split_1_Data,"ICD_DSM","ensemble","ICD_DSM","Ethnicity","")
#plot filename
plotfile_name =  "_Ethnicity"
#plot save
ggsave(paste(figs_dir,project_id,project_name,Sub_prefix,Split_n_post,plotfile_name,"_boxplot.png",sep=""))

#2. Gender
f_plots_posthoc(Split_1_Data,"ICD_DSM","ensemble","ICD_DSM","Gender","")
#plot filename
plotfile_name =  "_Gender"
#plot save
ggsave(paste(figs_dir,project_id,project_name,Sub_prefix,Split_n_post,plotfile_name,"_boxplot.png",sep=""))
...
#4. Medication
f_plots_posthoc(filter(Split_1_Data,Phenotype!="Control"),"ICD_DSM","ensemble","ICD_DSM","Medication","")
#plot filename
plotfile_name =  "_Medication"
#plot save
ggsave(paste(figs_dir,project_id,project_name,Sub_prefix,Split_n_post,plotfile_name,"_boxplot.png",sep=""))
...
rcorr(as.matrix(filter(Split_1_Data,ICD_DSM=="Other_Psychosis")[,c(6,34)]), type="pearson")
rcorr(as.matrix(filter(Split_1_Data,ICD_DSM=="Schizophrenia")[,c(6,34)]), type="pearson")
rcorr(as.matrix(filter(Split_1_Data,ICD_DSM=="Control")[,c(6,34)]), type="pearson")
...
#make dfs 
#corDFScz<-filter(anovaDF,ICD_DSM =="Schizophrenia" & ensemble <= 0.5)
#corDFOPSy<-filter(anovaDF,ICD_DSM !="Schizophrenia"& ensemble <= 0.5)
corDFScz<-filter(anovaDF,ICD_DSM =="Schizophrenia" )
corDFOPSy<-filter(anovaDF,ICD_DSM !="Schizophrenia")
#calculate correlations
# data frame 
rcorr(as.matrix(corDFScz[,c(13+n,34)]), type="pearson") # 13 is 1 before panss. So n goes down the line. Stupid hack. Try to avoid. 
rcorr(as.matrix(corDFOPSy[,c(13+n,34)]), type="pearson") # 13 is 1 before panss. So n goes down the line. Stupid hack. Try to avoid. 
...
table(Split_1_Data_nocon$dsmiv)
?resid
...
ggplot(filter(Split_1_Data_nocon), aes(x=dsmiv,y=ensemble, colour=dsmiv)) +
  geom_boxplot(notch=F)+
  geom_jitter() + 
  coord_flip()+
  theme(
...
#plot filename
plotfile_name =  "_dsmiv"
#plot save
ggsave(paste(figs_dir,project_id,project_name,Sub_prefix,Split_n_post,plotfile_name,"_boxplot.png",sep=""))

ggplot(Split_1_Data_nocon, aes(x=icd10,y=ensemble, colour=icd10)) +
  geom_boxplot(notch=F)+
  geom_jitter() + 
  coord_flip()+
  scale_x_discrete()+
  theme(
      ...
      axis.ticks.y=element_blank(),)
#plot filename
plotfile_name =  "_icd10"
#plot save
ggsave(paste(figs_dir,project_id,project_name,Sub_prefix,Split_n_post,plotfile_name,"_boxplot.png",sep=""))



ggplot(Split_1_Data_nocon, aes(x=reorder(icd10, ensemble, FUN=median),y=ensemble, colour=reorder(icd10, ensemble, FUN=median))) +
  geom_boxplot(notch=F)+
  geom_jitter() + 
  coord_flip()+
  scale_x_discrete()+
...
ggplot(filter(Split_1_Data_nocon), aes(x=dsmiv,y=glmnet, colour=dsmiv)) +
  geom_boxplot(notch=F)+
  geom_jitter() + 
  coord_flip()+
  facet_wrap(~Ethnicity)+
...
ggplot(filter(Split_1_Data_nocon, Ethnicity == "White" | Ethnicity == "Black"), aes(x=dsmiv,y=ensemble, colour=Ethnicity)) +
  geom_boxplot(notch=F)+
  geom_jitter() + 
  coord_flip()
  #facet_wrap(~icd10)+
...
dim(Split_1_Data_nocon)
Split_1_Data_nocon$Accuracy
ggplot(Split_1_Data_nocon, aes(Percentage, colour = ICD_DSM)) +
  geom_density(alpha=0.01)+
  #facet_wrap(~ Model)+
  labs(x = plot_xlabel_density)
...
ggplot(filter(Split_1_Data, Ethnicity == "White" | Ethnicity == "Black"), aes(x=ICD_DSM,y=glmnet, colour=Ethnicity)) +
  geom_boxplot(notch=F)+
  geom_jitter() + 
  coord_flip()+
  #facet_wrap(~icd10)+
...
t_title="_GAP_all"
latex="_latex"
#filename_demo=paste(output_dir,project_id,project_name,Sub_prefix,Split_n_post,t_title,"_Demographics.csv",sep="")
...
mldata=Split_1_Data
listVars <- c("Gender","Age", "Ethnicity","Medication")
catVars <- c("Ethnicity","Medication")
table1 <- CreateTableOne(vars = listVars, data = mldata, factorVars = catVars,strata=c("ICD_DSM"),includeNA = T)
table1print<-print(table1, quote = FALSE, noSpaces = TRUE, printToggle = FALSE)
#not latex
...
#stargazer latex tables
table1sg<-stargazer(table1print[,1:4],summary=FALSE, rownames=T)
