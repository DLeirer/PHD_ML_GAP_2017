...
Gx_DF_input<-GX_DF_adj[,c("Phenotype",GAP_Fdata$TargetID)]
...
1) Linear discriminant analysis (lda, lda)
2) Generalized Linear Model (logistic, glm)
3) Stochastic Gradient Boosting (gbm, gbm)
4) Lasso and Elastic-Net Regularized Generalized Linear Models (glmnet, glmnet)
5) Naive Bayes (nb, nb)
6) k-Nearest Neighbors (knn, knn)
7) Support Vector Machines with Linear Kernel (svmLinear, svmLinear2)
8) Support Vector Machines with Polynomial Kernel (svmPoly, svmPoly)
9) Support Vector Machines with Radial Basis Function Kernel (svmRad, svmRadial)
10) Random Forest (rf, rf)
11) C5.0 (c50, c5.0)
...
#modelID<-c("lda","glm","gbm","glmnet","nb","knn","svmLinear2","svmPoly","svmRadial","rf","C5.0","parRF","AdaBoost.M1","AdaBag","elm","nnet","avNNet","pcaNNet")
modelID<-c("lda","glm","gbm","glmnet","nb","knn","svmLinear2","svmPoly","svmRadial","rf","C5.0","parRF","AdaBag","elm","nnet","avNNet","pcaNNet")
new_mlalgs<-c()
...
for(i in 1:length(modelID)){
  tryCatch({
    ...
    List_of_models[[modelID[i]]] <- train(Phenotype~., data=Gx_DF_input[1:50,1:10], method=modelID[i], metric=metric, preProc=c("center", "scale"), trControl=control)
  }, 
