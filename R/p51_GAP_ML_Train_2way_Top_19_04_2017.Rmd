...
highlyCorrelated<-cor_fun(Gx_DF_1_locHS[,-c(1)],cor_cutoff=0.8)
highlyCorrelated
...
#For saving files. 
Sub_prefix = "_R1_"
Plotting_prefix = paste(project_id,project_name,Sub_prefix, sep="") 

#load ML data
load(file=paste(output_dir,project_id,project_name,"_all_models",".rdata",sep=""))

#Select functioning models
modelID_fine<-c("glmnet","nb","svmPoly","svmRadial","rf","parRF","gbm","C5.0","pcaNNet","elm")
modellist<-List_of_models[modelID_fine]
...
f_title="Average accuracy of 5x3-fold cross validation for 14 Models"
ggplot(accuracy_plot,
  aes(x=Accuracy_results,y=Algorithm,color=Algorithm)) +
  geom_point()+ 
  ggtitle(f_title)
ggsave(paste(figs_dir,Plotting_prefix,s_title,"_Free.png",sep=""))
...
f_title="Accuracy of 5x3-fold cross validation for 14 Models"
...
#Control for algorithms
control <- trainControl(method="repeatedcv", number=3, repeats=2, sampling="down",classProbs = TRUE, savePredictions="final") ## Steve added sampling argument. READ!!
control <- trainControl(method="repeatedcv", number=5, repeats=2, sampling="down",classProbs = TRUE,summaryFunction=twoClassSummary)

modelID<-c("gbm","glmnet","nb","knn","rf","C5.0","pcaNNet")
modelID<-c("gbm","glmnet","nb","knn","rf")
...
for(i in 1:length(modelID)){
  tryCatch({
    ...
    List_of_models[[modelID[i]]]<-"NA: Probable error"
    set.seed(seed)
    List_of_models[[modelID[i]]] <- train(Phenotype~., data=training, method=modelID[i], metric=metric, preProc=c("center", "scale"), trControl=control)
    print(List_of_models[[modelID[i]]])
  }, 
  error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
...
#my_control <- trainControl(method="repeatedcv", number=5, repeats=2, sampling="down",summaryFunction=twoClassSummary)
...
#Ensemble caret
set.seed(seed)
model_list <- caretList(Phenotype~., data=training, trControl=my_control,metric=metric, preProc=c("center", "scale"),methodList=c("knn","glmnet","rpart"))
#check
model_list
